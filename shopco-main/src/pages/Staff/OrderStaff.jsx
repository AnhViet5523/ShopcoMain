...
/**
 * Component OrderStaff - Quản lý và hiển thị danh sách đơn hàng cho nhân viên
...
const OrderStaff = () => {
  ...
  const [activeItem, setActiveItem] = useState('orderStaff');
  ...
  // Danh sách các mục trong sidebar dành cho nhân viên
  const sidebarItems = [
  ...
  /**
   * Hàm lấy dữ liệu từ API bao gồm tất cả đơn hàng và đơn hàng bị hủy
   */
  const fetchData = async () => {
    try {
      ...
      if (response && response.$values && Array.isArray(response.$values)) {
        ...
        // Lấy thông tin chi tiết về các mặt hàng trong mỗi đơn hàng
      ...
      const cancelResponse = await adminService.getCancelledOrders();
      console.log('Response từ API cancelled orders:', cancelResponse);

      if (cancelResponse && cancelResponse.$values && Array.isArray(cancelResponse.$values)) {
        setCancelledOrders(cancelResponse.$values);
      } else {
        console.error('Dữ liệu trả về không đúng định dạng:', cancelResponse);
      }
    } catch (error) {
  ...
  /**
   * Tính toán tổng số trang cho phân trang
   */
   ...
   * @param {number} value - Số trang mới
   */
   ...
   * @returns {Array} - Danh sách đơn hàng của trang hiện tại
   */
   ...
   */
  const handleApproveCancellation = async (cancelRequestId, orderId) => {
    try {
      console.log('Approving cancellation for order:', orderId);
      // Gọi API để chấp nhận yêu cầu hủy, sử dụng orderId thay vì cancelRequestId
      await adminService.approveCancellation(orderId);
      
      // Hiển thị thông báo thành công
      alert('Đã chấp nhận yêu cầu hủy đơn hàng!');
      
      // Cập nhật lại danh sách đơn hàng
      fetchData();
    } catch (error) {
      console.error('Error approving cancellation:', error);
      alert('Có lỗi xảy ra khi chấp nhận yêu cầu hủy đơn hàng!');
    }
  };
   ...
   * @param {string} cancelRequestId - ID của yêu cầu hủy đơn hàng
   */
  const handleRejectCancellation = async (cancelRequestId, orderId) => {
    try {
      console.log('Rejecting cancellation for order:', orderId);
      // Gọi API để từ chối yêu cầu hủy, sử dụng orderId thay vì cancelRequestId
      await adminService.rejectCancellation(orderId);
      ...
      alert('Có lỗi xảy ra khi từ chối yêu cầu hủy đơn hàng!');
    }
  };
  ...
  const handleDelivered = async (orderId) => {
    try {
      ...
      if (response) {
        // Hiển thị thông báo thành công
        alert('Đã cập nhật trạng thái đơn hàng thành "Đã giao"!');
        ...
        const updatedOrders = orders.map(order => {
          if (order.orderId === orderId) {
            return {
              ...
              lastUpdated: currentTime // Thêm thởi gian cập nhật
            };
          }
          return order;
        });
                            ...
                            getCurrentPageItems().map((order, index) => (
                              activeTab === 'Đơn hàng bị hủy' ? (
                                      ...
                                      {order.orderStatus === 'Cancelling' && (
                                        <>
                                          ...
                                          <Button
                                            ...
                                            color="success"
                                            size="small"
                                            onClick={() => handleApproveCancellation(order.cancelRequest?.cancelRequestId, order.orderId)}
                                          ...
                                          <Button
                                            ...
                                            color="error"
                                            size="small"
                                            onClick={() => handleRejectCancellation(order.cancelRequest?.cancelRequestId, order.orderId)}
                                ...
                                <tr key={order.orderId}>
                                  ...
                                  <td>{order.name || '-'}</td>
                                  <td>{order.phoneNumber || '-'}</td>
                                  <td className="product-name-cell">{order.items?.$values.map(item => item.productName).join(', ')}</td>
                                  <td>
                                    <strong>{order.totalAmount?.toLocaleString('vi-VN')} đ</strong>
                                  </td>
                                  <td>{formatOrderDate(order.orderDate)}</td>
                    ...
                    <Table size="small">
                      <TableBody>
                        ...
                        <TableRow>
                          ...
                          <TableCell>{formatOrderDate(orderDetail.orderDate)}</TableCell>
                        </TableRow>
                        <TableRow>
