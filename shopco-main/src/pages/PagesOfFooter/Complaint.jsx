...
const Complaint = () => {
  ...
  const [selectedContent, setSelectedContent] = useState('complaint');
      ...
      case 'complaint':
        return (
          <Typography variant="body1" sx={{ mb: 2 }}>
            {/* Existing content for complaints */}
          </Typography>
        );
      // Add more cases for other content sections as needed
      default:
        return null;
    }
  };
                  ...
                  <List>
                    {section.items.map((item) => (
                      <ListItem 
                        ...
                        key={item.text} 
                        onClick={() => {
                          item.action(); // Call the original action
                          setSelectedContent(item.text === 'Góp ý - Khiếu nại' ? 'complaint' : 'otherContent'); // Set selected content
                        }}
                      >
