...
export default function ProductDetail() {
  ...
  const handleBuyNow = async () => {
    try {
      ...
      // Nếu đã login, tiếp tục với quy trình mua hàng
      const buyNowResponse = await orderService.buyNow(user.userId, product.productId, quantity);
      ...
      if (currentCartResponse && currentCartResponse.orderId) {
        // Tạo query params với thông tin user đã login
        const queryParams = new URLSearchParams({
          orderId: currentCartResponse.orderId,
          name: user.firstName + ' ' + user.lastName || '',
          email: user.email || '',
          phone: user.phoneNumber || '',
          // Sử dụng địa chỉ từ thông tin user đã login
